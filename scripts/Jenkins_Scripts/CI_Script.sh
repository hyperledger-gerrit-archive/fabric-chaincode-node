...
export CONTAINER_LIST=(ca orderer peer0.org1 couchdb cli)
...
echo "ERROR !!!! --------> $1 <---------"
        ...
        for CONTAINER in ${CONTAINER_LIST[*]}; do
        	docker logs -f $CONTAINER.example.com >& $CONTAINER_LIST.log
        done
...
sdk_E2e_Tests() {
        ...
        echo "#################################################################"
        ls -l /tmp/fabric-shim
        echo "###############&&&&&&&&"

        docker images | grep hyperledger && docker ps -a
        gulp test-e2e err_Check "ERROR!!! test-e2e failed"

        DEVMODE=true gulp channel-init err_Check "ERROR!!! test-devmode-cli failed"
