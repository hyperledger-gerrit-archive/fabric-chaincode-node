...
module.exports.Transaction = function Transaction(commit = true) {
    return (target, propertyKey) => {
        logger.info('@Transaction args', target.name, propertyKey, commit);
        ...
        logger.debug(`Transaction ${target.name} -> ${propertyKey} params`, paramNames);

        const description = '';
        const contextType = target.createContext().constructor;

        logger.debug(`Transaction ${target.name} context type`, contextType);
        ...
        const parameters = paramTypes.filter((paramType, paramIdx) => {
            ...
            if (filter) {
                logger.debug(`Transaction ${target.name} -> ${propertyKey} ignoring param as matched context type`, paramNames[paramIdx]);
...
module.exports.Returns = function Returns(returnType) {
    return (target, propertyKey) => {
        logger.info('@Returns args', target.name, propertyKey, returnType);
