...
describe('smartcontract.js',()=>{
    ...
    describe('#constructor',()=>{
        ...
        it('should create with default namespace  ',()=>{
            let sc = new SmartContract();
            expect(sc.namespace).to.equal('smartcontract');
            ...
            let sc1 = new SmartContract('');
            expect(sc1.namespace).to.equal('smartcontract');
            expect(sc1.$getNamespace()).to.equal('smartcontract');

            let sc2 = new SmartContract('  ');
            expect(sc2.namespace).to.equal('smartcontract');
            expect(sc2.$getNamespace()).to.equal('smartcontract');
        });

        it('should create with the name specified',()=>{
            let sc1 = new SmartContract('brain.size.planet.smart');
            expect(sc1.namespace).to.equal('brain.size.planet.smart');
            expect(sc1.$getNamespace()).to.equal('brain.size.planet.smart');

            let sc2 = new SmartContract('   somewhat.padded.out ');
    ...
    describe('#_isFunction',()=>{
        let sc;
        beforeEach('create temporary smart contract',()=>{
