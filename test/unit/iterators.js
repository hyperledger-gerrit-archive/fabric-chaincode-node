...
const client = require('fabric-shim/lib/handler.js');

test('Iterator constructor tests', (t) => {
	let mockClient = sinon.createStubInstance(client);
	let historyQI = new HistoryQueryIterator(mockClient, 'tx1', 'somebytes');
	let queryQI = new StateQueryIterator(mockClient, 'tx2', 'someotherbytes');
	t.equal(historyQI.type, 'HISTORY', 'Test History iterator construction');
	t.equal(historyQI.handler, mockClient, 'Test History iterator construction');
	...
	t.equal(queryQI.handler, mockClient, 'Test Query iterator construction');
...
test('next: Simple data 1st entry, no more', (t) => {
	...
	let historyQI = new HistoryQueryIterator(mockClient, 'tx1', historyResponse);
	let queryQI = new StateQueryIterator(mockClient, 'tx2', queryResponse);
...
test('next: reached end, has more', (t) => {
	let mockClientHistory = sinon.createStubInstance(client);
	let mockClientQuery = sinon.createStubInstance(client);
	...
	let historyQI = new HistoryQueryIterator(mockClientHistory, 'tx1', historyResponse1);
	let queryQI = new StateQueryIterator(mockClientQuery, 'tx2', queryResponse1);
	...
	mockClientHistory.handleQueryStateNext.resolves(historyResponse2);
	mockClientQuery.handleQueryStateNext.resolves(queryResponse2);
	historyQI.currentLoc = 2;
	queryQI.currentLoc = 2;
	...
	let historyProm = historyQI.next()
		.then((result) => {
			t.equal(mockClientHistory.handleQueryStateNext.calledOnce, true, 'Test history called handleQueryStateNext');
			t.deepEqual(mockClientHistory.handleQueryStateNext.firstCall.args, [2, 'tx1']);
	...
	let queryProm = queryQI.next()
		.then((result) => {
			t.deepEqual(mockClientQuery.handleQueryStateNext.firstCall.args, [3, 'tx2']);
			t.equal(mockClientQuery.handleQueryStateNext.calledOnce, true, 'Test history called handleQueryStateNext');
...
test('close: Test close', (t) => {
	...
	let mockClientHistory = sinon.createStubInstance(client);
	let mockClientQuery = sinon.createStubInstance(client);
	let historyQI = new HistoryQueryIterator(mockClientHistory, 'tx1', historyResponse);
	let queryQI = new StateQueryIterator(mockClientQuery, 'tx2', queryResponse);
	mockClientHistory.handleQueryStateClose.resolves('something1');
	mockClientQuery.handleQueryStateClose.resolves('something2');
	let allproms = [];
	let hp = historyQI.close().then((res) => {
		t.equal(res, 'something1', 'Test history close resolves to some data');
		t.equal(mockClientHistory.handleQueryStateClose.calledOnce, true, 'Test history calls handle querystateclose once');
		t.deepEqual(mockClientHistory.handleQueryStateClose.firstCall.args, [1, 'tx1'], 'Test history calls handlestateclose with correct parameters');
	});
	let qp = queryQI.close().then((res) => {
		t.equal(res, 'something2', 'Test query close resolves to some data');
		t.equal(mockClientQuery.handleQueryStateClose.calledOnce, true, 'Test query calls handle querystateclose once');
		t.deepEqual(mockClientQuery.handleQueryStateClose.firstCall.args, [2, 'tx2'], 'Test query calls handlestateclose with correct parameters');
	});
	allproms.push(hp);
	allproms.push(qp);
	Promise.all(allproms).then(() => {
