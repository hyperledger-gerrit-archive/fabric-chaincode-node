...
describe('Chaincode CRUD', () => {
    ...
    describe('GET', () => {
        ...
        it('should get single key value', async function () {
            ...
            const payload = JSON.parse(await utils.query(suite, 'org.mynamespace.crud:getKey', ['string']));
        ...
        it('should get a single key containing json data', async function() {
            this.timeout(SHORT_INC);
            const payload = JSON.parse(await utils.query(suite, 'org.mynamespace.crud:getKey', ['jsonkey1']));
            expect(JSON.parse(payload.result)).to.deep.equal({value: 'value1'});
        });

        it('should get a single composite key value', async function () {
            this.timeout(SHORT_INC);
            const payload = JSON.parse(await utils.query(suite, 'org.mynamespace.crud:getCompositeKey', ['ann', 'black']));
            expect(payload.result).to.equal('annblack');
        });

        it('should return a list of results from a partial composite key', async function() {
            this.timeout(SHORT_INC);
            const payload = JSON.parse(await utils.query(suite, 'org.mynamespace.crud:getPartialCompositeKey', ['ann']));
        ...
        it('should return all keys between key1 and key3', async function() {
            this.timeout(SHORT_INC);
            const payload = JSON.parse(await utils.query(suite, 'org.mynamespace.crud:getKeysByRange', ['key1', 'key4']));
            expect(payload.result).to.deep.equal(['value1', 'value2', 'value3']);
        });

        it('should return a paginated list', async function() {
            this.timeout(SHORT_INC);
            const payload1 = JSON.parse(await utils.query(suite, 'org.mynamespace.crud:getStateByRangeWithPagination', ['key1', 'key4', '2', 'key2']));
            expect(payload1.result).to.deep.equal(['value2', 'value3']);
            const payload2 = JSON.parse(await utils.query(suite, 'org.mynamespace.crud:getStateByRangeWithPagination', ['key1', 'key4', '3', '']));
        ...
        it('should return a state from a partial composite key', async function() {
            this.timeout(SHORT_INC);
            const payload = JSON.parse(await utils.query(suite, 'org.mynamespace.crud:getStateByPartialCompositeKey', ['name~color', 'ann']));
            expect(payload.result).to.deep.equal(['annblack', 'annred', 'annyellow']);
        });

        it('should return a paginated list from partial composte key', async function() {
            this.timeout(SHORT_INC);
            const payload = JSON.parse(await utils.query(suite, 'org.mynamespace.crud:getStateByPartialCompositeKeyWithPagination', ['name~color', '1']));
    ...
    describe('PUT', () => {
        it('should add a key', async function() {
            ...
            const payload = JSON.parse(await utils.query(suite, 'org.mynamespace.crud:getKey', ['newKey1']));
        ...
        it('should add a composite key', async function() {
            ...
            const payload = JSON.parse(await utils.query(suite, 'org.mynamespace.crud:getCompositeKey', ['tim', 'green']));
        ...
        it('should update an existing key', async function() {
            ...
            const payload = JSON.parse(await utils.query(suite, 'org.mynamespace.crud:getKey', ['key1']));
    ...
    describe('DELETE', () => {
        it('should delete a key', async function() {
            ...
            const payload = JSON.parse(await utils.query(suite, 'org.mynamespace.crud:getKey', ['key2']));
