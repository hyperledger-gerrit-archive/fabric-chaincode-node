...
class testTS implements ChaincodeInterface {
    ...
    testCert(cert: X509.Certificate) {
        let subject: X509.Subject = cert.subject;
        let issuer: X509.Issuer = cert.issuer;
        let notAfter: string = cert.notAfter;
        let notBefore: string = cert.notBefore;
        let fingerPrint: string = cert.fingerPrint;
        let publicKey: string = cert.publicKey;
        let sigAlg: string = cert.signatureAlgorithm;
        ...
        let organisationalUnitName: string = subject.organizationalUnitName;
        let postcode: string = subject.postalCode;
        let stateName: string = subject.stateOrProvinceName;
        let address: string = subject.streetAddress;
    }
