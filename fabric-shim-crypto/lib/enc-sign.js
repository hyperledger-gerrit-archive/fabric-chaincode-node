...
class ChaincodeCryptoLibrary {
	 ...
	 */
	constructor(stub) {
		let tmap = stub.getTransient(); // string <-> byte[]
		let key = tmap.get(ENCRYPT_KEY);
		let iv = tmap.get(INIT_VECTOR); // optional

		if (!iv)
			// transaction proposal did not include an IV, generate one
			iv = crypto.randomBytes(16); // 128-bit IV for AES (block size)
		else
			iv = iv.toBuffer();
	...
	sign(message) {
		...
		// module './ecdsa/key.js'
		let signKey = this._ecdsa.keyFromPrivate(this.signKey._key.prvKeyHex, 'hex');
	...
	verify(signature, message) {
		...
		let pubKey = this._ecdsa.keyFromPublic(this.signKey.getPublicKey()._key.pubKeyHex, 'hex');
		// note that the signature is generated on the hash of the message, not the message itself
		let successful = pubKey.verify(hash(message), signature);
		return {
			ok: successful,
