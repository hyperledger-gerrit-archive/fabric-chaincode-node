...
let ECDSAKey = ShimCrypto.__get__('ECDSAKey');
...
describe('enc-sign', () => {
	describe('ChaincodeCryptoLibrary', () => {
		...
		it ('should set key values when init vector in map', () => {
			let mockCreateCipher = sandbox.stub(crypto, 'createCipheriv').returns('some cipher');
			let mockCreateDecipher = sandbox.stub(crypto, 'createDecipheriv').returns('some decipher');

			let sc = newShimCrypto(['iv', 'encryptKey']);
		...
		it ('should set key values when init vector not in map', () => {
			...
			let mockRandomBytes = sandbox.stub(crypto, 'randomBytes').returns('some random bytes');
			let mockCreateCipher = sandbox.stub(crypto, 'createCipheriv').returns('some cipher');
			let mockCreateDecipher = sandbox.stub(crypto, 'createDecipheriv').returns('some decipher');

			let sc = newShimCrypto(['encryptKey']);
		...
		describe('sign', () => {
			...
			it ('should sign the message', () => {
				let savePreventMalleability = ShimCrypto.__get__('_preventMalleability');
				let saveHash = ShimCrypto.__get__('hash');

				let _preventMalleabilityStub = sinon.stub().returns({
					toDER: sinon.stub().returns('some DER')
				});
				ShimCrypto.__set__('_preventMalleability', _preventMalleabilityStub);

				let hashStub = sinon.stub().returns('some hash');
		...
		describe('verify', () => {
			...
			it ('should return an error if not malleable', () => {
				let saveCheckMalleability = ShimCrypto.__get__('_checkMalleability');

				let _checkMalleabilityStub = sinon.stub().returns(false);
				...
				let result = sc.verify('some sig', 'some message');
			...
			it ('should return ok true for verfied signed message', () => {
				...
				let _checkMalleabilityStub = sinon.stub().returns(true);
				ShimCrypto.__set__('_checkMalleability', _checkMalleabilityStub);
	...
	describe('hash', () => {
		it ('should hash a message', () => {
			...
			let digestStub = sinon.stub().returns('some digest');

			let createHashStub = sinon.stub(crypto, 'createHash').returns({
				update: updateStub,
				digest: digestStub
			});

			let result = ShimCrypto.__get__('hash')('some message');
	...
	describe('importKey', () => {
		let importKey = ShimCrypto.__get__('importKey');

		let KEYUTIL = ShimCrypto.__get__('KEYUTIL');
		let saveMakeRealPem = ShimCrypto.__get__('makeRealPem');
		...
		it ('should throw an error if unable to parse PEM string', () => {
			let getKeyStub = sinon.stub(KEYUTIL, 'getKey').throws('some error');
		...
		it ('should throw an error if key does not have type', () => {
			let getKeyStub = sinon.stub(KEYUTIL, 'getKey').returns('some key');
		...
		it ('should throw an error if key type not EC', () => {
			let getKeyStub = sinon.stub(KEYUTIL, 'getKey').returns({
				type: 'some type'
			});
	...
	describe('makeRealPem', () => {
		let makeRealPem = ShimCrypto.__get__('makeRealPem');
...
-----END -----`;
			...
			let myChangedCert = `-----BEGIN CERTIFICATE-----
-----BEGIN EC CERT-----
MIIB4TCCAWcCCQDNvJ6OSzyk+jAKBggqhkjOPQQDAjBaMQswCQYDVQQGEwJnYjEM
MAoGA1UECAwDaWJtMQwwCgYDVQQHDANpYm0xDDAKBgNVBAoMA2libTETMBEGA1UE
CwwKYmxvY2tjaGFpbjEMMAoGA1UEAwwDaWJtMB4XDTE4MDcxMjE0MjAyOVoXDTI4
...
-----END CERTIFICATE-----`;
	...
	describe('_preventMalleability', () => {
		let _preventMalleability = ShimCrypto.__get__('_preventMalleability');
		...
		it ('should return sig for a low s', () => {
			let halfOrdersForCurve = ShimCrypto.__get__('halfOrdersForCurve');
			...
			let result = _preventMalleability(sig ,{
				name: 'secp256r1'
			});
		...
		it ('should handle s larger than half', () => {
			let subStub = sinon.stub().returns('some sub');

			let saveBN = ShimCrypto.__get__('BN');

			let mockBN = sinon.spy(() => {
				let bnStubInstance = sinon.createStubInstance(saveBN);
			...
			let result = _preventMalleability(sig, curveParams);
	...
	describe('_checkMalleability', () => {
		let _checkMalleability = ShimCrypto.__get__('_checkMalleability');
		...
		it ('should throw an error if signature object fails to load correctly (no r)', () => {
			let saveSig = ShimCrypto.__get__('Signature');

			let mockSignature = sinon.spy(() => {
		...
		it ('should throw an error if signature object fails to load correctly (no s)', () => {
			...
			let mockSignature = sinon.spy(() => {
				let sigStubInstance = sinon.createStubInstance(saveSig);
		...
		it ('should return false when s is larger than half the order', () => {
			...
			let cmpStub = sinon.stub().returns(1);
		...
		it ('should return false when s is lower than half the order', () => {
			...
			let cmpStub = sinon.stub().returns(0);
