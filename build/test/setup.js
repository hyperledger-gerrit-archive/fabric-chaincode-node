...
gulp.task('clean-up-crypto', async () => {
    await fs.remove(path.join(networkDir, 'crypto-material', 'crypto-config'));
    await fs.remove(path.join(networkDir, 'crypto-material', 'channel.tx'));
    await fs.remove(path.join(networkDir, 'crypto-material', 'core.yaml'));
    await fs.remove(path.join(networkDir, 'crypto-material', 'genesis.block'));
    await fs.remove(path.join(networkDir, 'crypto-material', 'mychannel.block'));
});
...
gulp.task('clean-up', gulp.series('clean-up-crypto', 'clean-up-chaincode'));
...
gulp.task('docker-clean', gulp.series('clean-up', () => {
    return gulp.src('*.js', {read: false})
        .pipe(shell([
            ...
            util.format('docker-compose -f %s down', fs.realpathSync(path.join(dockerComposeDir, 'docker-compose-cli.yaml'))),
...
gulp.task('generate-config', gulp.series('cli-ready', () => {
    ...
    return gulp.src('*.js', {read: false})
        .pipe(shell([
            ...
            util.format(
                'docker exec cli configtxgen -profile TwoOrgsOrdererGenesis -outputBlock %s/genesis.block',
                dockerCfgPath
            ),
            util.format(
...
gulp.task('channel-create', gulp.series('docker-setup', () => {
