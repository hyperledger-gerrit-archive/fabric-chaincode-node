...
gulp.task('generate-config', gulp.series('cli-ready', () => {
    ...
    return gulp.src('*.js', {read: false})
        .pipe(shell([
            ...
            util.format(
                'docker exec cli configtxgen -profile TwoOrgsOrdererGenesis -outputBlock %s/genesis.block -channelID not_%s',
                dockerCfgPath,
                channelName
            ),
            util.format(
...
gulp.task('channel-create', gulp.series('docker-setup', 'delay', () => {
    ...
    return gulp.src('*.js', {read: false})
        .pipe(shell([
            'echo "#############\nCRYPTO MATERIAL FILES START\n#############"',
            'ls -l build/test/network/crypto-material',
            'echo "#############\nCRYPTO MATERIAL FILES END\n#############"',
            'echo "#############\nORDERER LOGS START\n#############"',
