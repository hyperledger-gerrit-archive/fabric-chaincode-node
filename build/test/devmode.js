...
gulp.task('dm-startup-chaincode', async (done) => {
    const script = util.format('docker exec org1_cli bash -c "ls -l %s; node %s/node_modules/fabric-shim/cli.js start --peer.address %s --chaincode-id-name %s --module-path %s"',
        '/opt/gopath/src/github.com/chaincode/scenario',
        peerAddress,
        'mysmartcontract:v0',
        '/opt/gopath/src/github.com/chaincode/scenario');
