...
gulp.task('test-devmode',['invokeAllFnsDm']);

/**
 * Invoke all the smart contract functions
 */
gulp.task('invokeAllFnsDm',(done)=>{
	const tasks = [
		...
		// install
		'dm-install_chaincode',

		// instantiate
		'dm-instantiate_chaincode',
		'delay',
	...
	console.log('=== Starting CLI Tests'); // eslint-disable-line
...
gulp.task('dm-copy-api', ['dm-copy-shim'], () => {
	// first ensure the chaincode folder has the latest shim code
...
gulp.task('dm-copy-shim-crypto', ['dm-copy-api'], () => {
	// first ensure the chaincode folder has the latest shim code
...
gulp.task('dm-copy-chaincode', ['dmlocalpublish'], () => {
...
gulp.task('dm-install_chaincode', () => {
	return gulp.src('*.js', {read: false})
		.pipe(shell([
...
gulp.task('dm-instantiate_chaincode', () => {
	return gulp.src('*.js', {read: false})
		.pipe(shell([
