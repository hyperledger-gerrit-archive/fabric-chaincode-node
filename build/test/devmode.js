...
gulp.task('dm-startup-chaincode', async (done) => {
    const script = util.format('docker exec org1_cli bash -c "apk add nodejs nodejs-npm python make g++; cd %s; npm install; npm rebuild; ls -laR; node_modules/.bin/fabric-chaincode-node start --peer.address %s --chaincode-id-name %s --module-path %s"',
