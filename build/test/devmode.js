...
gulp.task('dm-startup-chaincode', async (done) => {
    const script = util.format('docker exec org1_cli bash -c "ls -l %s/node_modules/.bin; %s/node_modules/.bin/fabric-chaincode-node start --peer.address %s --chaincode-id-name %s --module-path %s"',
        '/opt/gopath/src/github.com/chaincode/scenario',
        '/opt/gopath/src/github.com/chaincode/scenario',
        peerAddress,
        'mysmartcontract:v0',
