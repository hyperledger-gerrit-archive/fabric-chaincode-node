...
gulp.task('test-e2e-install-v0', ['copy-chaincode'], () => {
	return gulp.src('*.js', {read: false})
		.pipe(shell([
			util.format('docker exec cli peer chaincode install -l node -n %s -v v0 -p %s --waitForEvent',
				CC_NAME,
...
gulp.task('test-e2e-instantiate-v0', ['test-e2e-install-v0'], () => {
	return gulp.src('*.js', {read: false})
		.pipe(shell([
			util.format('docker exec cli peer chaincode instantiate -o %s %s -l node -C %s -n %s -v v0 -c %s -P %s --waitForEvent',
...
gulp.task('test-e2e-invoke-v0-test12', ['test-e2e-invoke-v0-test11'], () => {
	const cmd = 'docker exec cli peer chaincode query %s -C %s -n %s -c %s --waitForEvent --transient ' +
				'\'{"encrypt-key":"MDEyMzQ1Njc4OTAxMjM0NTY3ODkwMTIzNDU2Nzg5MDE=","iv":"MDEyMzQ1Njc4OTAxMjM0NQ=="}\'';
