...
class ChaincodeStub {
	 ...
	 * event producer. Note that the block event gets delivered to the listerners regardless of the
	 * status of the included transactions (can be either valid or invalid), so client applications
	 * is responsible for checking the validity code on each transaction. Consult each SDK's documentation
	 * for details.
	 ...
	 * U+0000 (nil byte) and U+FFFF (biggest and unallocated code point). The resulting composite key can be
	 ...
	 * U+0000 (nil byte) and U+FFFF (biggest and unallocated code point).<br><br>
	 *
