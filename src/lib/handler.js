		...
		stream.on('data', function (msg) {
			...
			if (state === STATES.Established) {
				if (msg.type === MSG_TYPE.READY) {
					logger.debug('Successfully established communication with peer node. State transferred to "ready"');
					state = STATES.Ready;
				} else {
			...
			if (state === STATES.Created) {
				if (msg.type === MSG_TYPE.REGISTERED) {
					logger.debug('Successfully registered with peer node. State transferred to "established"');
	...
	if (input) {
		...
		if (stub) {
			...
			if (resp.status >= Stub.RESPONSE_CODE.ERROR) {
				...
				logger.debug(util.format('[%s-%s]Calling chaincode %s() succeeded. Sending COMPLETED message back to peer',
					msg.channel_id, shortTxid(msg.txid), method));
