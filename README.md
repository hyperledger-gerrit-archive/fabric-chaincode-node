...
gulp test-e2e
```
* to run the integration tests with TLS disabled:
```
TLS=false DEVMODE=false gulp channel-init
TLS=false gulp test-e2e
```

**NOTE:** if both DEVMODE & TLS are enabled you would need to generate TLS Certs for the chaincode
