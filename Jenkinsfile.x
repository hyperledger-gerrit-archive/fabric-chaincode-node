      ...
      stage("Fetch Patchset") { // fetch gerrit refspec on latest commit
          try {
              ...
              sh '''
                 ...
                 cd $PROJECT && git checkout "$GERRIT_BRANCH" && git fetch origin "$GERRIT_REFSPEC"
                 cd $PROJECT_DIR && git clone --single-branch -b $GERRIT_BRANCH git://cloud.hyperledger.org/mirror/fabric
              '''
              }
          }
          catch (err) {
