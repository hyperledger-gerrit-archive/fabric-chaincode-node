...
class Bootstrap {
    ...
    static loadAndValidateMetadata(metadataPath) {
        ...
        const metadataString = fsOld.readFileSync(metadataPath).toString();
        const schemaString = fsOld.readFileSync(path.join(rootPath, '../../fabric-contract-api/schema/contract-schema.json')).toString();
