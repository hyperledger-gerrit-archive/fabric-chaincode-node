...
class Bootstrap {
    ...
    static loadAndValidateMetadata(metadataPath) {
        ...
        const metadataString = originalFS.readFileSync(metadataPath).toString();
        const schemaString = originalFS.readFileSync(path.join(rootPath, '../../fabric-contract-api/schema/contract-schema.json')).toString();
