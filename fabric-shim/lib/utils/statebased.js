...
class KeyEndorsementPolicy {
    ...
    _getPolicyFromMspId() {
        ...
        mspIds.forEach((mspId, i) => {
            const mspRole = new fabprotos.common.MSPRole({
                role: this.orgs[mspId],
                mspIdentifier: mspId
            });
            const principal = new fabprotos.common.MSPPrincipal({
        ...
        const allOf = new fabprotos.common.SignaturePolicy.NOutOf({
            n: mspIds.length,
            rules: sigsPolicies
        });
        ...
        const spe = new fabprotos.common.SignaturePolicyEnvelope({
            version: 0,
            rule: noutof,
