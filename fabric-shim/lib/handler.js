...
class ChaincodeSupportClient {
	...
	async handleGetStateByRange(collection, startKey, endKey, metadata, channel_id, txId) {
...
function handleGetQueryResult(handler, res, method) {
	...
	if (payload.metadata) {
		...
		const metadata = _serviceProto.QueryMetadata.decode(payload.metadata);
		// FIXME: this might be a peer side bug, bookmark was serialized to 'nil', change this to null
		if (metadata && metadata.pageSize === 0 && metadata.bookmark === 'nil') {
...
function parseResponse(handler, res, method) {
	if (res.type === MSG_TYPE.RESPONSE) {
		...
		case 'GetQueryResult':
			return handleGetQueryResult(handler, res, method); // _serviceProto.GetQueryResult
