...
describe('SystemContract', () => {
    ...
    describe('#GetMetadata', () => {
        ...
        it ('validate and return the metadata', async () => {
            ...
            const metadata = await meta.loadAndValidateMetadata(metadataPath);
        ...
        it ('fail to validate and throw an error', async () => {
            ...
            await expect(meta.loadAndValidateMetadata(metadataPath)).to.eventually.be.rejectedWith(Error, 'Contract metadata does not match the schema: some string');
            sinon.assert.calledTwice(readFileStub);
            SystemContract.__set__('Ajv', ajvOriginal);
