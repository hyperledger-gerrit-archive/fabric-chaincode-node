...
describe('Chaincode', () => {
	...
	describe('Start()', () => {
		...
		beforeEach(() => {
			...
			Chaincode.__set__('opts', {'$0': 'fabric-chaincode-node'});
		});
		...
		it ('should start when passed init and invoke', () => {
			...
			Chaincode.__set__('opts', {'$0': 'fabric-chaincode-node', 'peer.address': 'localhost:7051', 'chaincode-id-name': 'mycc'});
		...
		it ('should delete unnecessary arguments passed to the CLI before passing on', () => {
			...
			Chaincode.__set__('opts', {'$0': 'fabric-chaincode-node', 'peer.address': 'localhost:7051', 'chaincode-id-name': 'mycc', 'some-other-arg': 'another-arg', 'yet-another-bad-arg': 'arg'});
		...
		it ('should call the builder function for chaincode command when call directly to shim.start', () => {
			Chaincode.__set__('opts', {});
			let mockBuilder = sinon.stub().returns({'peer.address': 'localhost:7051', 'chaincode-id-name': 'mycc'});

			let mockChaincodeCmd = {
				builder: () => {
					return mockBuilder();
				}
			};

			let chaincodeCmd = Chaincode.__get__('ChaincodeCommand');
			Chaincode.__set__('ChaincodeCommand', mockChaincodeCmd);
			let handlerClass = Chaincode.__get__('Handler');
			let chat = sinon.stub(handlerClass.prototype, 'chat');
			...
			expect(chat.calledOnce).to.be.ok;
			expect(mockBuilder.calledOnce).to.be.ok;
			...
			chat.restore();
			Chaincode.__set__('ChaincodeCommand', chaincodeCmd);
		});
